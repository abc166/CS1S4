...
{
// FIRST FIT
int available[20] = {0}; //available acts as bool array to check if
the given mem block is occupied
// 0 - memory is available, 1- Memory is occupied
printf("\nFIRST FIT:\n");
printf("\nProcess No\tProcess Size\tBlock No.\n");
for (int i = 0; i < n_processes; i++)
{
int tmp_flag = 0; //To check for not allocated after inner loop
ends
...
void best_fit()
{
int available[20] = {0}; //available acts as bool array to check
if the given mem block is occupied
// 0 - memory is available, 1- Memory is occupied
// Best Fit
printf("\nBEST FIT:\n");
printf("\nProcess No.\tProcess Size\tBlock No.\n");
...
if (available[j] == 0 && processes[i] <= mem_blocks[j])
{
if (min_avail_block >= mem_blocks[j] || min_avail_block
